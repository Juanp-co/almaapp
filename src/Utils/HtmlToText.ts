...
export default function HtmlToPlainText(htmlText, styleConfig?: any) {
  // define default styleConfig
  let linkProcess = null;
  let imgProcess = null;
  let headingStyle = 'underline'; // hashify, breakline
  ...
  if (bodyEndMatch) tmp = tmp.substring(0, bodyEndMatch.index);

  const bodyStartMatch = tmp.match(/<body[^>]*>/i);
  if (bodyStartMatch) tmp = tmp.substring(bodyStartMatch.index + bodyStartMatch[0].length, tmp.length);
  ...
  const createListReplaceCb = () => {
    return (match, listType, listAttributes, listBody) => {
      ...
      if (listAttributes && /start="([0-9]+)"/i.test(listAttributes)) {
        liIndex = parseInt((/start="([0-9]+)"/i.exec(listAttributes)[1]), 10) - 1;
      }
      return '<p>' + listBody.replace(/<li[^>]*>(((?!<li[^>]*>)(?!<\/li>).)*)<\/li>/gi, function (str, listItem) {
        let actSubIndex = 0;
  ...
  // handle lists
  if (listStyle === 'linebreak') tmp = tmp.replace(/<\/?ul[^>]*>|<\/?ol[^>]*>|<\/?li[^>]*>/gi, '\n');
  ...
  // handle headings
  if (headingStyle === 'linebreak') tmp = tmp.replace(/<h([1-6])[^>]*>([^<]*)<\/h\1>/gi, '\n$2\n');
